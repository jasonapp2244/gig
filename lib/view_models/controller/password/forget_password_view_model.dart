  ...
  void forgetPasswordApi() {
    ...
    _api.forgetPasswordApi(data).then((value) {
      ...
      if (value['status'] == true) {
        Utils.snakBar('OTP Sent', value['message'] ?? 'One Time Password sent to you email');
        Get.offAllNamed(RoutesName.loginScreen);
      }
      else {
        print("Forget Password failed: $value");
        Utils.snakBar('Forget Password', value['error'] ?? 'Something went wrong');
      }
      ...
      // Utils.snakBar('Error', 'The selected email is invalid.');
    });
  }
