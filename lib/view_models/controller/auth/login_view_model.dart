...
import '../../../repository/auth_repository/google_auth_repository.dart';
...
class LoginVewModel extends GetxController {
  ...
  /// Login with Google data using existing API
  Future<void> _loginWithGoogleData(GoogleUserData googleUser) async {
    try {
      print('🔄 Calling login API with Google data...');

      String fcmToken = await _getToken() ?? '';

      // Prepare data for login API with Google parameters
      Map<String, dynamic> data = {
        'email': googleUser.email,
        'service_provider': 'google', // Required parameter
        'service_provider_id': googleUser.uid, // Required parameter
        'fcm_token': fcmToken,
        // Additional Google data for your backend
        // 'name': googleUser.displayName,
        // 'photo': googleUser.photoURL,
        // 'google_id_token': googleUser.idToken,
        // 'google_access_token': googleUser.accessToken,
      };

      print('📤 Sending Google login data: $data');

      // Call the same login API
      ...
      if (value['status'] == true) {
        ...
        await storage.write(key: 'auth_method', value: 'google');
        await storage.write(
          key: 'google_user_photo',
        ...
        String errorMsg = value['message'] ?? 'Google Sign-In failed';
        ...
        if (value['errors'] != null && value['errors'] is Map) {
          ...
          Utils.snakBar('Google Sign-In Failed', errorMsg);
        }
      }
    } catch (error) {
      print('❌ Error in Google login API call: $error');
      Utils.snakBar('Error', 'Failed to complete Google Sign-In');
    }
  }
